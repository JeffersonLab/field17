...
c        separated by ,'s.
c
c     Joerg Reinhold 19-April-2003
c     copied field03.f to current03.f (this file)
c     use Makefilecurrent03 to compile and link
c     Added calculation of dipole current. This allows to set by current.
c
...
c are normalized to 6 GeV
      ...
      write(*,*) 'Q1 Bdl = ',bl_q1,', Q2 Bdl = ',bl_q2,', Q3 Bdl = ',
     $     bl_q3
...
c Tell the user what setting he wants.
      if(iargcount.ge.1) then
         print '(3(f9.3,","),f11.7,",",f9.3)',I_Q1*sign_HMS*-1.
     $        ,I_Q2*sign_HMS,I_Q3*sign_HMS*-1.,B_D *sign_HMS
     $        ,I_D *sign_HMS
      else
